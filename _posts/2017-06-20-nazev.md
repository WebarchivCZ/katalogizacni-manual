...
Pokud pramen popisu obsahuje **další názvovou informaci**, zapíše se do $b. Podpole $a je ukončeno #: (mezera dvojtečka). Podpole $b začíná vždy malým písmenem, pokud nezačíná slovem, které se píše s velkým písmenem (řídí se jazykovými pravidly příslušného jazyka). Další názvová informace se přebírá pouze ze stejného pramene popisu jako hlavní název, je-li uvedeno jinde, píše se do poznámky.
